 ...
 * 
 * PROPÓSITO:
 * - Implementa el patrón DAO para gestión de inscripciones
 * - Maneja relaciones complejas entre estudiantes, cursos y períodos
 * - Implementa validaciones de negocio críticas (correlatividades, créditos)
 * - Demuestra consultas SQL complejas con múltiples JOINs
 * - Gestiona estados de inscripción y evaluaciones
 * 
 * CONCEPTOS DEMOSTRADOS:
 * - Patrón DAO: encapsulación de acceso a datos complejos
 * - Consultas SQL avanzadas: JOINs múltiples para datos relacionados
 * - Validaciones de negocio: reglas académicas complejas
 * - Mapeo objeto-relacional: conversión de ResultSet a objetos
 * - Manejo de transacciones: operaciones atómicas críticas
 * - Clases internas: DTOs para datos complejos
 * 
 * @author Ludmila Martos
 */
public class InscripcionDAOImpl implements DAO<Integer, Inscripcion> {

    // IMPLEMENTACIÓN DE MÉTODO DE BÚSQUEDA BÁSICA
    /**
    ...
    public Inscripcion buscarPorID(Integer id) throws SQLException {
        ...
        try (Connection conn = ConexionDB.getConexion();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return mapearInscripcion(rs);  // Convertir ResultSet a objeto Inscripcion
                }
            }
        }
        return null;
    ...
    // IMPLEMENTACIÓN DE MÉTODO DE BÚSQUEDA AVANZADA CON JOINs
    /**
     * Busca una inscripción por ID con todos sus detalles relacionados.
     * Demuestra consulta SQL compleja con múltiples JOINs para obtener datos completos.
     */
    public InscripcionDetalle buscarDetallesPorID(Integer id) throws SQLException {
        // Consulta SQL compleja con múltiples JOINs
        // Demuestra cómo obtener datos relacionados de múltiples tablas
        String sql = "SELECT " +
